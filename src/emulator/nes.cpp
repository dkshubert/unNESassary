...
void NES::run(double time)
{
    ...
    // almost certainly results in less fidelitous clock timing, which means this approach
    // for determining the # of cycles to execute since the last run might need to be revised
    // when this project is further along. This will probably work reasonably well for the
    // time being, though.
    ...
    _logger.write(
        LogLevel::trace,
        fmt::format(
            "time: {}, previous time: {}, num cycles to run: {}",
            time,
            _previousTime,
            numCyclesToRun
        )
